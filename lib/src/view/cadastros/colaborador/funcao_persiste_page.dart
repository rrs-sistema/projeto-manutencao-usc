...
class FuncaoPersistePage extends StatefulWidget {
  final Funcao? funcao;
  final String? title;
  final String? operacao;

  const FuncaoPersistePage({Key? key, this.funcao, this.title, this.operacao})
      : super(key: key);

  @override
  FuncaoPersistePageState createState() => FuncaoPersistePageState();
}

class FuncaoPersistePageState extends State<FuncaoPersistePage> {
    ...
    if (!form.validate()) {
      ...
      gerarDialogBoxConfirmacao(context, Constantes.perguntaSalvarAlteracoes,
        ...
        if (widget.operacao == 'A') {
          await Sessao.db.funcaoDao.alterar(funcao!);
        } else {
          await Sessao.db.funcaoDao.inserir(funcao!);
        }
        if (!mounted) return;
  ...
  void _excluir() {
    gerarDialogBoxExclusao(context, () async {
      await Sessao.db.funcaoDao.excluir(funcao!);
      if (!mounted) return;
      Navigator.of(context).pop();
