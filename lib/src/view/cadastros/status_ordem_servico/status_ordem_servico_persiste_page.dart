...
class StatusOrdemServicoPersistePage extends StatefulWidget {
  final StatusOrdemServico? status;
  final String? title;
  final String? operacao;

  const StatusOrdemServicoPersistePage(
      {Key? key, this.status, this.title, this.operacao})
      : super(key: key);
...
class LocalPersistePageState extends State<StatusOrdemServicoPersistePage> {
    ...
    if (!form.validate()) {
      ...
      gerarDialogBoxConfirmacao(context, Constantes.perguntaSalvarAlteracoes,
        ...
        if (widget.operacao == 'A') {
          await Sessao.db.statusOrdemServicoDao.alterar(status!);
        } else {
          await Sessao.db.statusOrdemServicoDao.inserir(status!);
        }
        if (!mounted) return;
  ...
  void _excluir() {
    gerarDialogBoxExclusao(context, () async {
      await Sessao.db.statusOrdemServicoDao.excluir(status!);
      if (!mounted) return;
      Navigator.of(context).pop();
